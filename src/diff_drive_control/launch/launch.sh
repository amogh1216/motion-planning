...
gnome-terminal \
  ...
  --tab --title="Diff Drive Brain Params" -e "bash -c 'sleep 9; ros2 param load /diff_drive_brain src/diff_drive_control/config/config.yaml; 
              ros2 param load /localization_node src/diff_drive_control/config/config.yaml; exec bash'"